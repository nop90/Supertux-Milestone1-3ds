...
void st_video_setup_sdl(void)
{
  if (use_fullscreen)
    {
      screen = SDL_SetVideoMode(SCREEN_W, SCREEN_H, 16, SDL_FULLSCREEN | SDL_CONSOLEBOTTOM | SDL_HWSURFACE); 
      if (screen == NULL)
        {
          fprintf(stderr,
    ...
    {
      screen = SDL_SetVideoMode(SCREEN_W, SCREEN_H, 16, SDL_HWSURFACE | SDL_CONSOLEBOTTOM );
...
//      if (open_audio(44100, AUDIO_S16, 2, 2048) < 0)
      if (open_audio(22050, AUDIO_S16, 1, 1024) < 0)
        {
          /* only print out message if sound or music
             was not disabled at command-line
           */
